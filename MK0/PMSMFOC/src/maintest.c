...
void AdcIsr(ADCHS_CHANNEL_NUM channel, uintptr_t context) {
    ...
    //    VIMoni.Ia_real = IphaseConvert(VIMoni.Ia);
    //    VIMoni.Ib_real = IphaseConvert(VIMoni.Ib);
    //    VIMoni.Ic_real = -VIMoni.Ia_real - VIMoni.Ib_real;
    //    VIMoni.Vdc_real = VdcConvert(VIMoni.Vdc);
    ...
    if (++TickCount >= 5) {
        ...
        DV_Plot(VIMoni.Ia, VIMoni.Ib, -VIMoni.Ia-VIMoni.Ib, VIMoni.Vdc);
        //DV_Plot(ia, ib, ic, vdc);
        //DV_Plot(duty[0], duty[1], duty[2], 0);
...
int main(void) {
    //    int32_t ia, ib, ic, ia_real, ib_real, ic_real;
    //    uint32_t vdc, vdc_real;
